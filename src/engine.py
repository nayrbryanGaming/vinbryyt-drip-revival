...
def generate_social_post():
    """Generates a community post using Groq."""
    if not groq_client: return "System Status: Online. #VinBryYT"
    
    post_types = ["Pantun", "Motivational Quote", "System Status"]
    post_type = random.choice(post_types)
    
    try:
        completion = groq_client.chat.completions.create(
            messages=[
                {"role": "system", "content": f"You are an AI community manager for VinBryYT. Generate a short {post_type} (max 50 words) about 'Revival' and digital art. Use Indonesian for Pantun, English for others. End with #Solana #DRiP #VinBryYT."},
                {"role": "user", "content": "Generate today's update."}
            ],
            model="llama-3.3-70b-versatile",
        )
        post = completion.choices[0].message.content
        with open("community_posts.log", "a", encoding="utf-8") as f:
            f.write(f"\n[{datetime.now()}] {post_type}:\n{post}\n")
...
def generate_hourly_drop():
    """Full drop: Image + Metadata + Blinks."""
    ...
    # 1. Groq Prompt Engineering
    try:
        prompt_completion = groq_client.chat.completions.create(
            messages=[
                {"role": "system", "content": "You are a master of technical art prompts. Transform the theme into a 200-word highly detailed prompt for FLUX.1. focus on 8k, zero distortion, cinematic lighting, and sharp textures."},
        ...
        technical_prompt = f"A masterpiece of {theme}, high fidelity, 8k"
